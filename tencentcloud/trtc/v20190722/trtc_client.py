...
class TrtcClient(AbstractClient):
    ...
    def CreateCloudTranscription(self, request):
        r"""API description: Enable the cloud transcription feature.
    ...
    def QueryAIMessageList(self, request):
        r"""input a task ID to query the list of AI conversation context information.

        :param request: Request instance for QueryAIMessageList.
        :type request: :class:`tencentcloud.trtc.v20190722.models.QueryAIMessageListRequest`
        :rtype: :class:`tencentcloud.trtc.v20190722.models.QueryAIMessageListResponse`

        """
        try:
            params = request._serialize()
            headers = request.headers
            body = self.call("QueryAIMessageList", params, headers=headers)
            response = json.loads(body)
            model = models.QueryAIMessageListResponse()
            model._deserialize(response["Response"])
            return model
        except Exception as e:
            if isinstance(e, TencentCloudSDKException):
    ...
    def StartAIConversation(self, request):
        r"""Start an AI conversation task. The AI Channel Robot joins the TRTC room and performs AI dialogue with specified members inside the room. Suitable for smart customer service, AI spoken language teacher, and other scenarios.
    ...
    def StartAITranscription(self, request):
        r"""Start up the transcription bot. The backend will pass the robot stream pulling to perform real-time speech recognition and deliver subtitles and transcription messages.
    ...
    def StartWebRecord(self, request):
        r"""This API is used to trigger a page recording task. Specify parameters such as the recording URL, recording resolution, and recording result storage in the API parameter.
    ...
    def TextToSpeech(self, request):
        r"""This API is used to perform text to speech.
    ...
    def TextToSpeechSSE(self, request):
        r"""This API is used to stream text-to-speech.
    ...
    def VoiceClone(self, request):
        r"""This API is used to clone sound.
